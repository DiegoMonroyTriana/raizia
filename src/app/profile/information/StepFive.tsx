    ...
    <Container title={information.stepsFive.title} >
      ...
      <div className="grid grid-cols-2 gap-5 place-items-center">
        {information.stepsFive.content.map((item) => (
          <div key={item.title}>
            <h3 className="text-prussianBlue/70 text-xl font-bold text-center mb-4">{item.title}</h3>
            <div className="flex flex-col gap-2 max-w-xs justify-center ">
              {item.values.map((value) => (
                <Button key={`${item.title}-${value}`} variant="outline" className="text-prussianBlue text-sm border-prussianBlue">{value}</Button>
              ))}
            </div>
          </div>
        ))}
